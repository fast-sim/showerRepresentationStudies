...
def filter_hits(hit_collection):
    """
    Filter EM contributions from a SimCalorimeterHitCollection.
    """
    ...
    for hit in hit_collection:
        ...
        filtered_contribs = [
            ...
            if c.getPDG() not in EM_PDGS
        ]

        n_em_removed += len(contributions) - len(filtered_contribs)
...
def put_collection(frame_out, col, name):
    """
    Copy the collection to the output frame.
    """
    try:
...
def main():
    ...
    if args.output is None:
        ...
        args.output = f"{base}_hadronic.root"
    ...
    for iev, frame_in in enumerate(reader.get("events")):
        ...
        for col_name in args.calo_collections:
            ...
            print(
                ...
                f"removed EM contrib: {stats['em_removed']}"
            )
